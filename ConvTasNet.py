...
class ConvTasNet(nn.Module): # final model
    def __init__(self, num_speakers, num_filters, kernel_size, num_channels, num_blocks):
        """
        Parameters:
        ...
        self.encoder = Encoder(num_filters, kernel_size)
        self.tcn = TCN(num_channels, num_blocks, kernel_size)
        self.decoder = Decoder(num_filters, kernel_size)
        self.mask_conv = nn.Conv1d(num_channels, num_speakers * num_filters, kernel_size=1)
    ...
    def forward(self, x):
        ...
        return torch.cat([self.decoder(masked_speakers[:, i]) for i in range(self.num_speakers)], dim=1) # Section C (5)
